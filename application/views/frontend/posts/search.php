...
<div class="container">
    <ul id="myList" class="viewallposts">
        ...
        <?php
            ...
            // print_r($viewallposts->status);exit;
        if( $viewallposts->status!="Fail" ){
        ...
        $likecount = 0;
        if((isset($likecount)) && (isset($get_like))){
          foreach ($get_like as $abcd) {
            if(isset($abcd)){
        ...
        $likeis = $firebase->get('/post_like/singlepost/'.$post_id); 
            $i++;
       ?>
